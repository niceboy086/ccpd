...
def train_model(model, criterion, optimizer, num_epochs=25):
    ...
    for epoch in range(epoch_start, num_epochs):
        ...
        for i, (XI, Y, labels, ims) in enumerate(trainloader):
            ...
            # print("*debug, xxx*, train: loss = ", loss.data[0])
            # print("*debug, xxx*, train: loss = ", loss, loss.data, loss.item(), loss.data.item())

            try:
                lossAver.append(loss.data.item())
            except:
                pass
        ...
        with open(args['writeFile'], 'a') as outF:
            outF.write('%s %s %s\n' % (epoch, sum(lossAver) / len(lossAver) if len(lossAver)>0 else 'NoLoss', time() - start))
...
if __name__ == '__main__':
    ...
    # print(model_conv)
    print("model_conv: number of params ", get_n_params(model_conv))
